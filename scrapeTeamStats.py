    ...
    if(len(final_ids) == 0):
        ...
        for i in range(0, len(final_ids)):
            ...
            if(len(stats[11]) == 1):
                ...
                with db:
                    try:
                        db.execute('''INSERT INTO teamStatsPuntReturns(team, the_date, no, yards, avg_yards, long_yards, td) VALUES(?,?,?,?,?,?,?)''', (team2, gdate,int(team1_punt_returns[1]),int(team2_punt_returns[2]),float(team2_punt_returns[3]),int(team2_punt_returns[4]),int(team2_punt_returns[5])))
                        db.commit()
                    except sqlite3.IntegrityError as err:
                        print err
            if(len(stats[12]) == 1):
