...
for i in range(0, len(away_teams)):
    try:
        with db:
            db.execute('''INSERT INTO NCFSBLines(away_team, home_team, line, spread, game_date, game_time) VALUES(?,?,?,?,?,?)''', (away_teams[i], home_teams[i], lines[i], spreads[i], today, date_time))
            db.commit()
...
for i in range(0, len(away_teams)):
    try:
        with db:
            db.execute('''INSERT INTO NCFteamlookup(covers_team, espn_abbr, espn_name) VALUES (?,?,?)''', (away_teams[i], None, None))
            db.execute('''INSERT INTO NCFteamlookup(covers_team, espn_abbr, espn_name) VALUES (?,?,?)''', (home_teams[i], None, None))
    except:
        print 'Record Exists'
